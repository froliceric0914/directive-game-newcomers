...
export function createVisit(dialog,{stop,onClose,onClue=()=>{},onNarrativeProgress=()=>{},onVisitLocation=()=>{},onChapterComplete=()=>{},canCompleteChapter=()=>false,nextChapterStep=()=>null,chapterForLocation=()=>null}){
 ...
 function cast(speaker,npcId,showKaga=true,showNpc=true,protagonist='kaga'){
 const npc=npcs[npcId],player=npcs[protagonist];
 ...
 function saveMemories(ids){const progress=readInvestigationProgress(narrative.chapterId),unlocked=[...new Set([...progress.unlockedMemories,...ids.filter(Boolean)])];if(unlocked.length!==progress.unlockedMemories.length){writeInvestigationProgress(narrative.chapterId,{unlockedMemories:unlocked});onNarrativeProgress()}}
 function setDayLocation(id,transition=false){const place=locations.map.locations.find(item=>item.id===investigationLocationId(id));if(!place)return;readingPlace=place;if(transition)dayNpc=locations.scenes[place.id]?.npcId??locations.venues[place.venueId??place.id]?.npcIds?.[0]??dayNpc;scene={...dayScene,...(locations.scenes[place.id]??{}),background:locations.scenes[place.id]?.background??null,curtain:place.name};q('#scene-title').textContent=place.name;}
 function optionalActions(){const progress=readInvestigationProgress(narrative.chapterId);return (narrative.day.optionalVisits??[]).filter(visit=>!progress.completedOptionalVisits.includes(visit.id)&&(!visit.availableAfterMemory||progress.unlockedMemories.includes(visit.availableAfterMemory))).map(visit=>`<button data-optional-location="${investigationLocationId(visit.locationId)}">${esc(visit.title)} →</button>`).join('')}
 function wireOptional(){dialog.querySelectorAll('[data-optional-location]').forEach(button=>button.onclick=()=>{closeScene();onVisitLocation(button.dataset.optionalLocation)})}
 function dayTalk(){
 const flow=conversation.flow,node=conversationNodes(flow)[nodeId];if(!node)return;
 const protagonist=resolveCharacterId(narrative.day.playerCharacter??'kaga')??'kaga';
 if(node.locationId)setDayLocation(node.locationId,true);
 const lines=node.lines??[],current=lines[nodeLine];
 const currentNpc=resolveCharacterId(current?.speaker),nodeNpc=lines.map(item=>resolveCharacterId(item.speaker)).find(id=>id&&id!==protagonist);
 const npcId=(currentNpc!==protagonist?currentNpc:null)??(current?.speaker==='shopkeeper'?dayScene.npcId:null)??nodeNpc??dayNpc??(flow.characters??[]).map(resolveCharacterId).find(id=>id&&id!==protagonist)??(conversation.optional?null:dayScene.npcId);
 if(npcId&&npcId!==protagonist)dayNpc=npcId;
 const foreground=()=>cast(resolveCharacterId(current?.speaker)??current?.speaker,npcId,true,!!npcs[npcId]&&npcId!==protagonist,protagonist);
 const advance=next=>{nodeId=next;nodeLine=0;nodeMemoryShown=false;dayTalk()};
 if(current){q('.scene-content').innerHTML=`<div class="conversation">${foreground()}<div class="dialogue-box"><div class="dialogue-label"><strong>${esc(current.speaker==='system'?'':current.speaker==='shop-clerk'?'店员':speakerLabel(current.speaker))}</strong><small>${nodeLine+1} / ${lines.length}</small></div><p aria-live="polite">${esc(current.text)}</p><div class="dialogue-actions"><button id="previous" ${nodeLine===0?'disabled':''}>上一句 <kbd>←</kbd></button><button class="primary" id="next">继续 → <kbd>E</kbd></button></div></div></div>`;q('#previous').onclick=()=>{nodeLine--;dayTalk()};q('#next').onclick=()=>{nodeLine++;dayTalk()};return}
 ...
 if(node.type==='memory'&&memory&&!nodeMemoryShown){q('.scene-content').innerHTML=`<div class="conversation">${foreground()}<article class="narrative-memory"><h3>${esc(memory.title)}</h3><p>${esc(memory.text)}</p><button class="primary" id="next">继续 →</button></article></div>`;q('#next').onclick=()=>{nodeMemoryShown=true;dayTalk()};return}
 if(node.choices?.length){q('.scene-content').innerHTML=`<div class="conversation">${foreground()}<div class="dialogue-box narrative-choice">${node.prompt?`<h3>${esc(node.prompt)}</h3>`:''}<div class="narrative-choices">${node.choices.map(choice=>`<button data-node-next="${choice.next}">${esc(choice.label)}</button>`).join('')}</div></div></div>`;q('.narrative-choices').onclick=event=>{const button=event.target.closest('[data-node-next]');if(button)advance(button.dataset.nodeNext)};return}
 if(node.type==='complete'){
  const progress=readInvestigationProgress(narrative.chapterId),optional=conversation.optional;
  if(optional?!progress.completedOptionalVisits.includes(optional.id):!progress.dayCompleted){writeInvestigationProgress(narrative.chapterId,optional?{completedOptionalVisits:[...progress.completedOptionalVisits,optional.id]}:{dayCompleted:true});onNarrativeProgress()}
  const feedback=node.feedback??flow.feedback;
  q('.scene-content').innerHTML=`<article class="narrative-day-complete"><h3>${esc(typeof feedback==='string'?feedback:feedback?.title)}</h3>${feedback?.text?`<p>${esc(feedback.text)}</p>`:''}<div class="narrative-choices"><button class="primary" id="next">${esc(node.actionLabel??'返回街区')}</button>${optionalActions()}</div></article>`;
  q('#next').onclick=closeScene;wireOptional();return;
 }
 if(node.next)advance(node.next);
 }
 function talk(){if(conversation){dayTalk();return}const lines=selectedDialogue?.lines??scene.dialogue??[['kaga','这里暂时没有更多可询问的内容。','加贺恭一郎']],current=lines[line],speaker=current[0],npc=npcs[scene.npcId],showKaga=selectedDialogue?selectedDialogue.participantIds.includes('kaga'):true,showNpc=selectedDialogue?selectedDialogue.participantIds.includes(scene.npcId):true,canGrant=dialogueState==='activeInvestigation'&&!!clue;q('.scene-content').innerHTML=`<div class="conversation">${cast(speaker,scene.npcId,showKaga,showNpc)}<div class="dialogue-box"><div class="dialogue-label"><strong>${esc(current[2]??npcs[speaker]?.name??speaker)}</strong><small>${line+1} / ${lines.length}</small></div><p aria-live="polite">${esc(current[1])}</p><div class="dialogue-actions"><button id="previous" aria-keyshortcuts="ArrowLeft" ${line===0?'disabled':''}>上一句 <kbd>←</kbd></button><button class="primary" id="next" aria-keyshortcuts="e ArrowRight">${line===lines.length-1?(canGrant?(notes?'查看手帐 →':'记入手帐 →'):'返回街道 →'):'继续对话 →'} <kbd>E</kbd></button></div></div></div>`;q('#previous').onclick=()=>{line--;talk()};q('#next').onclick=()=>{if(line<lines.length-1){line++;talk()}else if(!canGrant)closeScene();else{notes=true;onClue(clue.id);show('notes')}};}
 function notebook(){
 if(narrative){const progress=readInvestigationProgress(narrative.chapterId),memories=chapterMemories(narrative);q('.scene-content').innerHTML=`<article class="scene-notes"><h3>${esc(narrative.title)}</h3>${progress.unlockedMemories.map(id=>memories[id]).filter(Boolean).map(memory=>`<section><strong>${esc(memory.title)}</strong><p>${esc(memory.text)}</p></section>`).join('')||'<p>听完对话，留意今天遇到的细节。</p>'}<div class="narrative-choices">${optionalActions()}<button id="note-action">返回对话 →</button></div></article>`;wireOptional();q('#note-action').onclick=()=>show('talk');return}
 q('.scene-content').innerHTML=`<article class="scene-notes"><h3>${esc(clue?.title??readingPlace.name)}</h3>${notes&&clue?`<span class="note-stamp">已记下</span><p>${esc(clue.text)}</p><div class="note-question">${esc(clue.question)}</div><p class="muted">${esc(clue.qualification)}</p>`:'<p>先听完这段对话，再把留意到的细节记下来。</p>'}<button class="primary" id="note-action">返回对话 →</button></article>`;q('#note-action').onclick=()=>show('talk');}
 ...
 stop();origin=readOnly;dialogueState=state;ambient=state==='ambient';readingPlace=place??locations.map.locations.find(item=>item.id==='sokaya');
 const activeChapter=chapterForLocation(readingPlace.id),baseScene=locations.scenes[readingPlace.id]??{},variant=baseScene.chapterVariants?.[String(activeChapter)],story=storyLocationForMap(readingPlace);
 narrative=loadChapter(activeChapter);conversation=!readOnly&&state==='activeInvestigation'?conversationForLocation(narrative,readingPlace.id,readInvestigationProgress(activeChapter)):null;
 const normalState=narrative&&state==='activeInvestigation'&&!conversation?'ambient':state;dialogueState=normalState;
 selectedDialogue=readOnly||conversation?null:selectStoryDialogue(readingPlace,normalState);
 const storyNpc=(selectedDialogue?.participantIds??story?.characters?.map(resolveCharacterId).filter(Boolean)??[]).find(id=>id!=='kaga');
 scene={npcId:storyNpc??baseScene.npcId,curtain:story?.name??readingPlace.name,background:conversation?null:'assets/locations/street.png',...baseScene,...(variant??{})};dayScene=scene;
 clue=clues.entries.find(item=>item.id===scene.clueId);line=0;notes=false;chapterId=activeChapter??story?.chapterIds?.[0]??readingForLocation(readingPlace.id)?.chapterIds[0]??1;fullChapter=!!readOnly&&!!place;
 shell();if(conversation){dayNpc=null;nodeId=conversation.flow.startNode;nodeLine=0;nodeMemoryShown=false;setDayLocation(conversation.optional?.locationId??narrative.day.locationId)}
 if(state!=='activeInvestigation')q('[data-view="notes"]').hidden=true;document.body.classList.add('in-scene');if(!dialog.open)dialog.showModal();show(readOnly?'read':'talk');
 }
 function openChapter(id,place=null){stop();conversation=null;narrative=null;origin=true;ambient=false;readingPlace=place;scene=place?locations.scenes[place.id]:null;clue=scene?clues.entries.find(item=>item.id===scene.clueId):null;line=0;notes=false;chapterId=id;fullChapter=true;shell();document.body.classList.add('in-scene');if(!dialog.open)dialog.showModal();show('read')}
 function openAmbient(place){open(false,place,'ambient')}
