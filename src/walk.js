...
$('#police-screen').insertAdjacentHTML('afterend',`<main class="evidence-screen" id="apartment-screen" hidden><button class="back-hub" data-screen="kodemmacho">← 返回小传马町</button><header class="evidence-heading"><small>CRIME SCENE · MINEKO APARTMENT</small><h2>${escape(minekoApartmentEvidence.name)}</h2><p>${escape(minekoApartmentEvidence.ui.description)}</p></header><nav class="evidence-tabs"><button data-evidence-tab="current" class="active">当前线索</button><button data-evidence-tab="resolved">已厘清</button></nav><section id="apartment-evidence"></section></main>`);
...
function renderApartment(){
 ...
 const evidence=visibleEvidence(minekoApartmentEvidence.evidence,current).filter(item=>(evidenceTab==='resolved'?'resolved':'question')===evidenceState(item));
 $('#apartment-evidence').innerHTML=evidence.length?`<div class="evidence-grid">${evidence.map(item=>{const id='ch'+String(item.unlockChapter).padStart(2,'0'),choice=savedReviews[id],investigated=!!choice,state=evidenceState(item);return `<details class="evidence-card ${state}"><summary><small>第 ${item.unlockChapter} 章 · ${escape(item.type)}</small><h3>${escape(item.title)}</h3><span>${state==='resolved'?'已厘清':'有疑点'}</span><p>${escape(item.before.summary)}</p></summary><div class="evidence-detail"><strong>发现时的问题</strong><ul>${item.before.questions.map(question=>`<li>${escape(question)}</li>`).join('')}</ul>${investigated?`<div class="investigation-notes"><strong>调查备注</strong><ul>${item.after.details.map(detail=>`<li>${escape(detail)}</li>`).join('')}</ul><strong>加贺的判断</strong><p>${escape(item.after.result)}</p></div>`:''}<p class="related-location">相关地点：${escape(item.investigation.label)}</p>${state==='question'?`<button class="primary" data-evidence-location="${item.unlockChapter}">沿着这条线索调查 →</button>`:''}</div></details>`}).join('')}</div>`:'<div class="evidence-empty">这一栏目前没有证物记录。</div>';
...
$('#apartment-screen').addEventListener('click',event=>{
 ...
 if(!place)return;
 player={...place.door};visual={...player};lastLocation='';showScreen('ningyocho');update();enterPlace(place);
});
