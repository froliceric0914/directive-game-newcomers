...
import {clues,locations,chapterEndReviews,ch1Day} from './game-data.mjs';
...
export function createVisit(dialog,{stop,onClose,onClue=()=>{},onCh1Progress=()=>{},onChapterComplete=()=>{},canCompleteChapter=()=>false,nextChapterStep=()=>null,chapterForLocation=()=>null}){
 let mode='talk',returnMode='talk',line=0,origin=false,ambient=false,notes=false,request=0,fullChapter=false,readingPlace=null,chapterId=1,scene=null,clue=null,selectedDialogue=null,dialogueState='ambient',ch1Conversation=false,ch1NodeId=null,ch1Line=0;
 let cleanupReading=()=>{};
 ...
 function cast(speaker,npcId,showKaga=true,showNpc=true){const npc=npcs[npcId]??npcs.naho;return `<div class="cast" data-location="${readingPlace.id}" style="--scene-background:url('${scene.background??'assets/locations/street.png'}')"><figure class="kaga ${speaker==='kaga'?'speaking':''}" ${showKaga?'':'hidden'}><div class="portrait"><img src="${portraitFor('kaga','conversation')}" alt="便装的加贺恭一郎"></div><figcaption>${esc(npcs.kaga.name)} <small>${esc(npcs.kaga.role)}</small></figcaption></figure><div class="shop-curtain" aria-hidden="true">${esc(scene.curtain)}</div><figure class="shopkeeper ${speaker===npcId?'speaking':''}" ${showNpc?'':'hidden'}><div class="portrait"><img src="${portraitFor(npcId)}" alt="${esc(npc.name)}" decoding="async"></div><figcaption>${esc(npc.name)} <small>${esc(npc.role)}</small></figcaption></figure></div>`}
 function ch1Talk(){const flow=ch1Day.activeConversation,node=flow.nodes[ch1NodeId];if(!node)return;
  if(node.type==='complete'){
   const progress=readCh1Progress();if(!progress.dayCompleted){writeCh1Progress({dayCompleted:true});onClue('senbei-jacket');onCh1Progress()}
   q('.scene-content').innerHTML=`<article class="ch1-day-complete"><small>${esc(node.feedback??flow.feedback)}</small><h3>${esc(flow.feedback)}</h3><p>把今天听到的话带回警局，慢慢整理。</p><button class="primary" id="next">${esc(node.actionLabel)}</button></article>`;q('#next').onclick=closeScene;return;
  }
  if(node.type==='memory'){
   const memory=ch1Day.memories[node.memoryId],progress=readCh1Progress(),memoryId=node.unlockMemory??node.memoryId;
   if(memoryId&&!progress.unlockedMemories.includes(memoryId)){writeCh1Progress({unlockedMemories:[...progress.unlockedMemories,memoryId]});onCh1Progress()}
   q('.scene-content').innerHTML=`<div class="conversation">${cast('kaga','naho')}<article class="ch1-memory"><small>今天记住的事</small><h3>${esc(memory.title)}</h3><p>${esc(memory.text)}</p><button class="primary" id="next">继续谈话 →</button></article></div>`;q('#next').onclick=()=>{ch1NodeId=node.next;ch1Line=0;ch1Talk()};return;
  }
  if(node.type==='choice'){
   q('.scene-content').innerHTML=`<div class="conversation">${cast('kaga','naho')}<div class="dialogue-box ch1-choice"><div class="dialogue-label"><strong>${esc(node.prompt)}</strong></div><div class="ch1-choices">${node.choices.map(choice=>`<button data-ch1-next="${choice.next}">${esc(choice.label)}</button>`).join('')}</div></div></div>`;q('.ch1-choices').onclick=event=>{const choice=event.target.closest('[data-ch1-next]');if(!choice)return;ch1NodeId=choice.dataset.ch1Next;ch1Line=0;ch1Talk()};return;
  }
  const lines=node.lines??[],current=lines[ch1Line],speaker=current.speaker,npcId=speaker==='kaga'?(lines.find(item=>item.speaker!=='kaga')?.speaker??'naho'):speaker;
  q('.scene-content').innerHTML=`<div class="conversation">${cast(speaker,npcId)}<div class="dialogue-box"><div class="dialogue-label"><strong>${esc(npcs[speaker]?.name??speaker)}</strong><small>${ch1Line+1} / ${lines.length}</small></div><p aria-live="polite">${esc(current.text)}</p><div class="dialogue-actions"><button id="previous" ${ch1Line===0?'disabled':''}>上一句 <kbd>←</kbd></button><button class="primary" id="next">${ch1Line===lines.length-1?'继续 →':'继续对话 →'} <kbd>E</kbd></button></div></div></div>`;
  q('#previous').onclick=()=>{ch1Line--;ch1Talk()};q('#next').onclick=()=>{if(ch1Line<lines.length-1)ch1Line++;else{ch1NodeId=node.next;ch1Line=0}ch1Talk()};
 }
 function talk(){if(ch1Conversation){ch1Talk();return}const lines=selectedDialogue?.lines??scene.dialogue??[['kaga','这里暂时没有更多可询问的内容。','加贺恭一郎']],current=lines[line],speaker=current[0],npc=npcs[scene.npcId],showKaga=selectedDialogue?selectedDialogue.participantIds.includes('kaga'):true,showNpc=selectedDialogue?selectedDialogue.participantIds.includes(scene.npcId):true,canGrant=dialogueState==='activeInvestigation'&&!!clue;q('.scene-content').innerHTML=`<div class="conversation">${cast(speaker,scene.npcId,showKaga,showNpc)}<div class="dialogue-box"><div class="dialogue-label"><strong>${esc(current[2]??npcs[speaker]?.name??speaker)}</strong><small>${line+1} / ${lines.length}</small></div><p aria-live="polite">${esc(current[1])}</p><div class="dialogue-actions"><button id="previous" aria-keyshortcuts="ArrowLeft" ${line===0?'disabled':''}>上一句 <kbd>←</kbd></button><button class="primary" id="next" aria-keyshortcuts="e ArrowRight">${line===lines.length-1?(canGrant?(notes?'查看手帐 →':'记入手帐 →'):'返回街道 →'):'继续对话 →'} <kbd>E</kbd></button></div></div></div>`;q('#previous').onclick=()=>{line--;talk()};q('#next').onclick=()=>{if(line<lines.length-1){line++;talk()}else if(!canGrant)closeScene();else{notes=true;onClue(clue.id);show('notes')}};}
 ...
 function open(readOnly=false,place=null,state='activeInvestigation'){stop();origin=readOnly;dialogueState=state;ambient=state==='ambient';readingPlace=place??locations.map.locations.find(item=>item.id==='sokaya');const activeChapter=chapterForLocation(readingPlace.id),baseScene=locations.scenes[readingPlace.id]??{},variant=baseScene.chapterVariants?.[String(activeChapter)],story=storyLocationForMap(readingPlace);ch1Conversation=!readOnly&&readingPlace.id==='sokaya'&&activeChapter===1&&state==='activeInvestigation'&&!readCh1Progress().dayCompleted;ch1NodeId=ch1Day.activeConversation.startNode;ch1Line=0;selectedDialogue=readOnly||ch1Conversation?null:selectStoryDialogue(readingPlace,state);const storyNpc=(selectedDialogue?.participantIds??story?.characters?.map(resolveCharacterId).filter(Boolean)??[]).find(id=>id!=='kaga');scene={npcId:storyNpc??baseScene.npcId??'kaga',curtain:story?.name??readingPlace.name,background:'assets/locations/street.png',...baseScene,...(variant??{})};clue=clues.entries.find(item=>item.id===scene.clueId);line=0;notes=false;chapterId=activeChapter??story?.chapterIds?.[0]??readingForLocation(readingPlace.id)?.chapterIds[0]??1;fullChapter=!!readOnly&&!!place;shell();if(state!=='activeInvestigation')q('[data-view="notes"]').hidden=true;document.body.classList.add('in-scene');if(!dialog.open)dialog.showModal();show(readOnly?'read':'talk')}
