...
import {clues,locations} from './game-data.mjs';
...
export function createVisit(dialog,{stop,onClose}){
 let mode='talk',line=0,origin=false,notes=false,request=0,fullChapter=false,readingPlace=null,chapterId=1,scene=null,clue=null;
 let cleanupReading=()=>{};
 const storage={getItem:key=>localStorage.getItem(key),setItem:(key,value)=>localStorage.setItem(key,value)};
 const q=s=>dialog.querySelector(s);
 const esc=s=>s.replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
 function shell(){dialog.className='visit-scene';dialog.setAttribute('aria-labelledby','scene-title');dialog.innerHTML=`<header class="scene-head"><div><small>人形町 · 暖帘之内</small><h2 id="scene-title">${origin?(readingPlace?esc(readingPlace.name)+' · 原著':'书页之间'):esc(readingPlace.name+' · '+readingPlace.kind)}</h2></div><button id="leave" aria-keyshortcuts="Escape">返回街道 <kbd>Esc</kbd></button></header><nav class="scene-tabs" aria-label="店内查看"><button data-view="talk" aria-keyshortcuts="t">对话 <kbd>T</kbd></button><button data-view="notes" aria-keyshortcuts="j">随身手帐 <kbd>J</kbd></button><button data-view="read" aria-keyshortcuts="r">阅读原著 <kbd>R</kbd></button></nav><section class="scene-content"></section><footer class="scene-foot">一间小店，一件寻常事。<span>新参者 · ${chapters[chapterId].label}</span></footer>`;
 q('#leave').onclick=closeScene;dialog.querySelectorAll('[data-view]').forEach(b=>{b.hidden=origin&&b.dataset.view!=='read';b.onclick=()=>show(b.dataset.view)});
 }
 ...
 function talk(){const lines=scene.dialogue,current=lines[line],speaker=current[0],npc=npcs[scene.npcId];q('.scene-content').innerHTML=`<div class="conversation"><div class="cast"><figure class="kaga ${speaker==='kaga'?'speaking':''}"><div class="portrait"><img src="${portraitFor('kaga','conversation')}" alt="便装的加贺恭一郎"></div><figcaption>${esc(npcs.kaga.name)} <small>日本桥署</small></figcaption></figure><div class="shop-curtain" aria-hidden="true">${esc(scene.curtain).split('').join('<br>')}</div><figure class="shopkeeper ${speaker===scene.npcId?'speaking':''}"><div class="portrait"><img src="${portraitFor(scene.npcId)}" alt="${esc(npc.name)}" decoding="async"></div><figcaption>${esc(npc.name)} <small>${esc(scene.npcRole)}</small></figcaption></figure></div><div class="dialogue-box"><div class="dialogue-label"><strong>${esc(npcs[speaker]?.name??speaker)}</strong><small>原著情节改写 · ${line+1} / ${lines.length}</small></div><p aria-live="polite">${esc(current[1])}</p><div class="dialogue-actions"><button id="previous" aria-keyshortcuts="ArrowLeft" ${line===0?'disabled':''}>上一句 <kbd>←</kbd></button><button class="primary" id="next" aria-keyshortcuts="e ArrowRight">${line===lines.length-1?(notes?'查看手帐 →':'记入手帐 →'):'继续对话 →'} <kbd>E</kbd></button></div></div></div>`;q('#previous').onclick=()=>{line--;talk()};q('#next').onclick=()=>{if(line<lines.length-1){line++;talk()}else{notes=true;show('notes')}};}
 function notebook(){q('.scene-content').innerHTML=`<article class="scene-notes"><small class="eyebrow">KAGA'S NOTEBOOK · ${String(locations.map.locations.findIndex(item=>item.id===readingPlace.id)+1).padStart(2,'0')}</small><h3>${esc(clue.title)}</h3>${notes?`<span class="note-stamp">已记下</span><p>${esc(clue.text)}</p><div class="note-question">${esc(clue.question)}</div><p class="muted">${esc(clue.qualification)}</p>`:'<p>先听完这段对话，再把留意到的细节记下来。</p>'}<button class="primary" id="note-action" aria-keyshortcuts="e">${notes?'展开这一段原著 →':'返回对话 →'} <kbd>E</kbd></button></article>`;q('#note-action').onclick=()=>show(notes?'read':'talk');}
 function closeScene(){cleanupReading();cleanupReading=()=>{};dialog.close()}
 function exitReading(){if(fullChapter&&mode==='read'){if(origin)closeScene();else{fullChapter=false;show('read')}}else closeScene()}
 async function read(){
 ...
 return {open(readOnly=false,place=null){stop();origin=readOnly;readingPlace=place??locations.map.locations.find(item=>item.id==='sokaya');scene=locations.scenes[readingPlace.id];clue=clues.entries.find(item=>item.id===scene.clueId);line=0;notes=false;chapterId=readingForLocation(readingPlace.id)?.chapterIds[0]??1;fullChapter=!!readOnly&&!!place;shell();document.body.classList.add('in-scene');if(!dialog.open)dialog.showModal();show(readOnly?'read':'talk')},key(key,repeat=false){
