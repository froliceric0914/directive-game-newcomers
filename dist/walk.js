...
function storySnapshot(){return storyProgress(savedReviews,storyStarted(),suspectReview.chapters.length)}
function currentStoryNode(){const snapshot=storySnapshot();return snapshot.complete?null:storyNodes[snapshot.currentIndex]}
function storyNodeState(node,index){if(savedReviews[node.id])return'completed';return index===storySnapshot().currentIndex?'current':'future'}
...
function storyProgressMarkup(compact=false){
 ...
 const currentCard=current?`<div class="current-story"><span>当前章节</span><strong>${esc(current.label)}</strong>${actions}</div>`:snapshot.complete?'<div class="current-story complete"><span>故事进度</span><strong>已完成全部章节</strong></div>':'<div class="current-story story-start"><span>故事起点</span><strong>先在峰子公寓查看田仓的名片</strong><button class="primary" data-story-apartment>前往峰子公寓</button></div>';
