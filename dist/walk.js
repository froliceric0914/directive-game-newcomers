...
let discoveredApartmentEvidence=new Set(),storyReadyReviews=new Set();
try{discoveredApartmentEvidence=new Set(JSON.parse(localStorage.getItem('discoveredApartmentEvidence')||'[]'))}catch{}
if(localStorage.getItem('storyProgress:minekoApartment')==='complete'&&!discoveredApartmentEvidence.has('insurance_material')){discoveredApartmentEvidence.add('insurance_material');localStorage.setItem('discoveredApartmentEvidence',JSON.stringify([...discoveredApartmentEvidence]));localStorage.removeItem('storyProgress:minekoApartment')}
...
const storyNodes=suspectReview.chapters.map((chapter,index)=>({id:chapter.chapterId,label:chapterLabelExceptions[chapter.chapterId]??chapter.title.replace(/的(?:女孩|小伙计|媳妇|狗|店员|社长|顾客|刑警)$/,''),chapterId:index+1,locationId:Object.keys(investigationChapterByLocation).find(id=>investigationChapterByLocation[id]===index+1)??null}));
function storyStarted(){return discoveredApartmentEvidence.has('insurance_material')||Object.keys(savedReviews).length>0||storyReadyReviews.size>0}
function readingReviews(){return {...savedReviews,...Object.fromEntries([...storyReadyReviews].map(id=>[id,'read']))}}
function storySnapshot(){return storyProgress(readingReviews(),storyStarted(),suspectReview.chapters.length)}
function currentStoryNode(){const snapshot=storySnapshot();return snapshot.complete?null:storyNodes[snapshot.currentIndex]}
function storyNodeState(node,index){if(readingReviews()[node.id])return'completed';return index===storySnapshot().currentIndex?'current':'future'}
function storyNodeReveal(index,snapshot){if(index<snapshot.currentIndex||index===snapshot.currentIndex)return'';const distance=index-(snapshot.currentIndex??-1);return distance===1?'preview-one':distance===2?'preview-two':'mystery'}
function storyProgressMarkup(compact=false){
 const snapshot=storySnapshot(),current=currentStoryNode();
 const track=storyNodes.map((node,index)=>{const state=storyNodeState(node,index),reveal=storyNodeReveal(index,snapshot),mystery=reveal==='mystery',number=String(index+1).padStart(2,'0'),connector=index<storyNodes.length-1?`<i class="${snapshot.started&&index<snapshot.currentIndex?'completed':''}" aria-hidden="true"></i>`:'';return `<button class="story-node ${state} ${reveal}" data-story-node="${node.id}" ${state==='future'?'disabled':''} ${state==='current'?'aria-current="step"':''} ${mystery?'aria-label="后续章节，尚未揭示"':''}>${state==='current'?`<span class="story-avatar"><img src="${portraitFor('kaga','map')}" alt="加贺当前所在章节"></span>`:''}<span class="story-dot" aria-hidden="true"></span><span class="story-label" ${mystery?'aria-hidden="true"':''}><small>${mystery?'&nbsp;':number}</small><strong>${mystery?'&nbsp;':esc(node.label)}</strong></span></button>${connector}`}).join('');
 const heading=`<header><div><small>STORY PROGRESS</small><strong>${compact?'故事进度':'峰子的足迹'}</strong></div><b>${snapshot.started?(snapshot.complete?snapshot.total:snapshot.currentIndex+1):0} / ${snapshot.total}</b></header>`;
 const place=current&&locations.find(item=>item.id===current.locationId);
 const actions=current?(compact?(place?`<button class="story-map-action" data-story-map>定位 ${esc(place.name)}</button>`:`<button class="primary" data-story-read>继续阅读</button>`):`<div class="story-actions"><button class="primary" data-story-read>继续阅读</button>${place?'<button class="story-map-action" data-story-map>前往地图探索</button>':''}</div>`):'';
 const currentCard=current?`<div class="current-story"><div class="current-story-label"><span>当前章节</span><strong> · ${esc(current.label)}</strong></div>${actions}</div>`:snapshot.complete?'<div class="current-story complete"><span>故事进度</span><strong>已完成全部章节</strong></div>':'<div class="current-story story-start"><span>故事起点</span><strong>先在峰子公寓查看田仓的名片</strong><button class="primary" data-story-apartment>前往峰子公寓</button></div>';
...
function renderStoryProgress(){
 ...
 const snapshot=storySnapshot(),state=$('#apartment-state'),copy=$('#apartment-copy');if(state&&copy){state.textContent=snapshot.started?'调查据点':'调查从这里开始';copy.textContent=snapshot.started?'随时返回查看已经发现的线索。':'查看峰子留下的物品与线索。'}
}
function stateForPlace(place){
 const chapter=investigationChapterByLocation[place.id];
 const investigationStarted=discoveredApartmentEvidence.has('insurance_material')||Object.keys(savedReviews).length>0,snapshot=storyProgress(savedReviews,investigationStarted,suspectReview.chapters.length),currentChapter=snapshot.currentChapter??(snapshot.complete?suspectReview.chapters.length+1:0);
...
function evidenceState(item){
 const id='ch'+String(item.unlockChapter).padStart(2,'0'),choice=savedReviews[id],chapter=reviewById[id];
...
function renderApartment(){
 ...
 $('#apartment-evidence').innerHTML=evidence.length?`<div class="evidence-grid">${evidence.map(item=>{const id='ch'+String(item.unlockChapter).padStart(2,'0'),choice=savedReviews[id],investigated=!!choice,state=evidenceState(item),place=locations.find(entry=>investigationChapterByLocation[entry.id]===item.unlockChapter);return `<details class="evidence-card ${state}" data-evidence-card="${item.id}"><summary><i>${icons[item.type]??'证'}</i><div><small>第 ${item.unlockChapter} 章</small><h3>${escape(item.title)}</h3><p>${escape(item.before.summary)}</p></div><span>${state==='resolved'?'已厘清':'有疑点'}</span></summary><div class="evidence-detail"><strong>发现时的问题</strong><ul>${item.before.questions.map(question=>`<li>${escape(question)}</li>`).join('')}</ul>${investigated?`<div class="investigation-notes"><strong>调查备注</strong><ul>${item.after.details.map(detail=>`<li>${escape(detail)}</li>`).join('')}</ul><strong>加贺的判断</strong><p>${escape(item.after.result)}</p></div>`:''}<p class="related-location">相关地点：${escape(item.investigation.label)}</p><div class="evidence-actions"><button data-evidence-read="${item.unlockChapter}">阅读全文</button>${place?`<button class="primary" data-evidence-location="${item.unlockChapter}">前往相关地点</button>`:''}</div></div></details>`}).join('')}</div>`:'<div class="evidence-empty">这一栏目前没有证物记录。</div>';
...
function readyReviewIds(){
 ...
 return suspectReview.chapters.map(chapter=>chapter.chapterId).filter(id=>ids.has(id));
}
function archiveEntries(){
 const entries=[],resolved=new Set();
 for(const chapter of suspectReview.chapters){
  const choice=savedReviews[chapter.chapterId];if(!choice)continue;
  const result=chapter.resolution[choice],decision=chapter.choices.find(item=>item.id===choice)?.label??choice;
  entries.push({id:chapter.chapterId,type:result.archiveAs,chapter:chapter.title,character:chapter.initialSuspect.name,decision,text:result.text});
  if(chapter.newDoubt)entries.push({id:chapter.newDoubt.id,type:'doubt',chapter:chapter.title,character:'未解问题',decision:'新增疑点',text:chapter.newDoubt.text});
  if(chapter.majorConclusion)entries.push({id:chapter.chapterId+'-major',type:'conclusion',chapter:chapter.title,character:chapter.initialSuspect.name,decision:'章节结论',text:chapter.majorConclusion});
  if(chapter.resolveDoubt)resolved.add(chapter.resolveDoubt);
 }
 return entries.map(entry=>({...entry,resolved:resolved.has(entry.id)}));
}
function renderPolice(){
 const ready=readyReviewIds();
 if(!selectedReview||!ready.includes(selectedReview))selectedReview=ready.find(id=>!savedReviews[id])??ready.at(-1)??null;
 const chapter=reviewById[selectedReview],choice=chapter&&savedReviews[selectedReview],result=choice&&chapter.resolution[choice];
 $('#suspect-review').innerHTML=chapter?`<div class="review-chapters">${ready.map(id=>`<button data-review="${id}" class="${id===selectedReview?'active':''}">${esc(reviewById[id].title)}</button>`).join('')}</div><article class="review-card"><small>${esc(chapter.chapterId.toUpperCase())} · ${esc(chapter.title)}</small><h3>${esc(chapter.initialSuspect.name)}</h3><p class="suspicion-reason">${esc(chapter.initialSuspect.reason)}</p>${choice?`<div class="review-resolution"><strong>${esc(chapter.choices.find(item=>item.id===choice)?.label??choice)}</strong><p>${esc(result.text)}</p>${chapter.nextSuspect?`<p class="next-suspect">新调查对象：${esc(chapter.nextSuspect.name)}<br>${esc(chapter.nextSuspect.reason)}</p>`:''}${chapter.finalState==='confirmed'&&choice==='confirm'?'<span class="confirmed">核心嫌疑人已确认</span>':''}</div>`:`<p class="review-question">${esc(chapter.reviewQuestion)}</p><div class="review-choices">${chapter.choices.map(item=>`<button data-choice="${item.id}">${esc(item.label)}</button>`).join('')}</div>`}</article>`:'<div class="review-empty"><h3>尚无可复核人物</h3><p>完成地点对话并取得相关事实后，章节嫌疑复核会出现在这里。</p></div>';
 if(choice){const button=document.createElement('button');button.className='reconsider';button.dataset.reconsider=selectedReview;button.textContent='重新判断';$('#suspect-review .review-resolution').append(button)}
 const entries=archiveEntries().filter(entry=>archiveFilter==='all'||entry.type===archiveFilter);
...
const state=l&&stateForPlace(l),visitLabels={active:'访问店家',visited_suspect:'再次询问',visited_cleared:'查看结论',ambient:'查看地点',locked:'尚未调查'};$('#visit').disabled=!l||state==='locked';$('#visit').innerHTML=(l?visitLabels[state]:'访问店家')+' <kbd>E</kbd>';$('#visit').title=l?(state==='locked'?'当前章节尚未开放这里':'访问'+l.name):'先走近店门或地标';const atStation=player.x===start.x&&player.y===start.y;const name=l?.name||(atStation?'人形町站':player.x>=10&&player.x<=12?'人形町站前道路':'甘酒横丁');$('#walking-place').textContent=name;$('#steps').textContent=steps+' 步';$('#walk-hint').textContent=message||(l?(state==='locked'?'这里尚未进入调查范围。':state==='visited_cleared'?'调查已完成，可查看保存的结论。':state==='visited_suspect'?'这里仍有疑点，可以再次询问。':state==='ambient'?'这是街区地点，可查看简短介绍。':'已到店门附近，按 E 开始调查。'):'方向键移动，走近店门查看地点。');document.querySelectorAll('.shop').forEach(el=>el.classList.toggle('near',el.dataset.shop===l?.id));if(lastLocation!==name){lastLocation=name;$('#location').textContent=name;$('#place-index').textContent=l?'地点 '+String(locations.indexOf(l)+1).padStart(2,'0'):'漫游途中';$('#place-description').textContent=l?l.kind:'沿着车站旁的十字路口，慢慢认识这条街。';$('#place-note').textContent=l?(state==='ambient'?'街区中的一处日常地点。':state==='visited_cleared'?'这处调查已经形成结论。':state==='visited_suspect'?'此前的判断仍留有疑点。':state==='locked'?'尚未取得进入这里的调查方向。':'当前章节的调查地点。'):'走近店门，手帐会翻到这家店。'}followPlayer();}
const movement=createMovement(start,step,position=>{
 player=position;steps++;update();
 const place=locations.find(l=>player.x===l.door.x&&player.y===l.door.y);
 if(place)enterPlace(place);
});
function tick(now){
 visual=movement.tick(now);
 $('#player').style.setProperty('--x',visual.x);$('#player').style.setProperty('--y',visual.y);if(activeScreen==='ningyocho')followPlayer();requestAnimationFrame(tick);
}
function enterPlace(place){
 const state=stateForPlace(place);if(state==='locked'){update('这里尚未进入调查范围。');return}
 if(state==='ambient'){scene.openAmbient(place);return}
 if(state==='visited_cleared'){const id='ch'+String(investigationChapterByLocation[place.id]).padStart(2,'0'),chapter=reviewById[id],choice=savedReviews[id];scene.openSummary(place,{suspectName:chapter.initialSuspect.name,text:chapter.resolution[choice].text});return}
 scene.open(false,place);
}
function visitPlace(){if(reader.open||movement.moving)return;const place=nearby(player);if(place)enterPlace(place);}
...
document.addEventListener('keydown',e=>{
 ...
 if(activeScreen!=='ningyocho'){if(e.key==='Escape'&&activeScreen!=='kodemmacho'){e.preventDefault();showScreen('kodemmacho')}return;}
...
$('#apartment-screen').addEventListener('click',event=>{
 const evidenceElement=event.target.closest('[data-evidence-id],[data-evidence-card]');
 if(evidenceElement){const evidenceId=evidenceElement.dataset.evidenceId??evidenceElement.dataset.evidenceCard;discoveredApartmentEvidence.add(evidenceId);localStorage.setItem('discoveredApartmentEvidence',JSON.stringify([...discoveredApartmentEvidence]));renderStoryProgress();renderMapStates()}
 ...
 const read=event.target.closest('[data-evidence-read]');if(read){scene.openChapter(Number(read.dataset.evidenceRead));return}
 const investigate=event.target.closest('[data-evidence-location]');if(!investigate)return;
 const chapter=Number(investigate.dataset.evidenceLocation),place=locations.find(item=>investigationChapterByLocation[item.id]===chapter);
 if(!place)return;
 showScreen('ningyocho');player={...place.door};visual={...player};lastLocation='';camera=null;update('当前线索指向：'+place.name);
});
